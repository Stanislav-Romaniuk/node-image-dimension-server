import { createServer } from 'node:http';
import { readFileSync } from 'node:fs';

const favicon = readFileSync('./favicon-32x32.png');

const limits = {
    minWidth: 50,
    minHeight: 50,
    maxWidth: 5000,
    maxHeight: 5000
};

function checkLimits(width, height, limits) {

    if (
        width < limits.minWidth ||
        width > limits.maxWidth ||
        height < limits.minHeight ||
        height > limits.maxHeight
    ) {
        throw new Error(
            `Allowed dimensions: width ${limits.minWidth}-${limits.maxWidth}, height ${limits.minHeight}-${limits.maxHeight}`
        );
    }
}

const server = createServer(async (req, res) => {
    try {
        console.log(req.url);

        if (req.url === '/favicon.ico') {
            res.writeHead(200, { 'Content-Type': 'image/png' });
            res.end(favicon);
            return;
        }

        const parts = req.url.split('/');

        if (parts[1] === 'image' && parts[2] && parts[3]) {
            const width = parseInt(parts[2]);
            const height = parseInt(parts[3]);

            if (isNaN(width) || isNaN(height)) {
                res.writeHead(400);
                res.end('Dimensions must be numbers. Format: /image/width/height');
                return;
            }

            checkLimits(width, height, limits);

            const inputImage = readFileSync('./photo.jpg');

            res.writeHead(200, { 'Content-Type': 'image/jpeg' });
            res.end(inputImage);

        } else {
            res.writeHead(200, { 'Content-Type': 'text/plain; charset=utf-8' });
            res.end('Use the format: http://127.0.0.1:3000/image/width/height');
        }

    } catch (err) {
        console.log(err.message);

        if (!res.headersSent) {
            res.writeHead(400, { 'Content-Type': 'text/plain' });
            res.end(err.message);
        }
    }
});

server.listen(3000, '127.0.0.1', () => {
    console.log('Listening on 127.0.0.1:3000');
});
